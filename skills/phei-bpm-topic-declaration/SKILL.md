...
- `brief`, `reader`, `feature`, and `compare` are dedicated BPM fields and must not be direct copies of planning-report sections. `brief` summarizes content, organization, and learning outcomes in 250-400 Chinese characters. `reader` uses one concise sentence for university majors, undergraduate/graduate students, and related practitioners. `feature` explains the content system, textbook organization, practice teaching, and frontier value in 250-400 Chinese characters. `compare` must compare exactly two verifiable books selected from live web-search context, state each book's strengths and relative coverage difference, and preserve the two source URLs in `compareSources`; never invent titles, URLs, sales, rankings, or quality claims.
